...
def pynetbox_connect():
    try:
        nb = pynetbox.api(config.url, token=config.token)
        logger.warning("-- Connection to Netbox API success --")
        return nb
    except Exception as e:
        logger.warning("Connection to Netbox failed.")
        logger.exception(e)
        raise ValueError("Connection to Netbox failed.")
...
def create_device_role_leaf(nb):
...
def create_device_role_spine(nb):
...
def create_manufacturer_juniper(nb):
    ...
    if not new_manufacturer_juniper:
        ...
        return new_manufacturer_juniper
        logger.warning("Adding new Manufacturer Juniper")


# Create Device Type 'vqfx10k'
def create_device_type_vqfx10k(nb, new_manufacturer_juniper):
...
def create_interfaces_template_for_device_type_vqfx10k(nb):
...
def main():
    nb = pynetbox_connect()
    create_site(nb)
    create_device_role_leaf(nb)
    create_device_role_spine(nb)
    new_manufacturer_juniper = create_manufacturer_juniper(nb)
    create_device_type_vqfx10k(nb, new_manufacturer_juniper)
    create_interfaces_template_for_device_type_vqfx10k(nb)
    create_platform_junos(nb)
    create_prefixes(nb)
